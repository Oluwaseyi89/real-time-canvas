 ...
 * `bg-slate-900/80 border border-slate-800 rounded-xl ...` field repeated
    ...
    <div className={containerClassName}>
      ...
      <div className="flex gap-2">
        <input
          ...
          className={`
            flex-1 w-full bg-slate-900/80 border rounded-xl px-3.5 py-2.5
            text-slate-100 text-xs outline-none transition-all
            placeholder:text-slate-600 font-medium
            focus:ring-1
            ${error
              ? 'border-rose-800/80 focus:border-rose-500/80 focus:ring-rose-500/50'
              : 'border-slate-800 focus:border-indigo-500/80 focus:ring-indigo-500/50'}
            ${className}
          `.trim().replace(/\s+/g, ' ')}
          {...rest}
        />
      ...
      {!error && hint && (
        <p id={`${inputId}-hint`} className="mt-1.5 text-[11px] text-slate-500">
          {hint}
        </p>
      )}
    </div>
