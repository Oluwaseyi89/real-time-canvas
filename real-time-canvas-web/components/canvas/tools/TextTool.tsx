      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative flex-1 min-w-[200px]">
          <input
            ...
            className="w-full px-3 py-1.5 bg-slate-900/80 border border-slate-700/80 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all font-sans"
            onKeyDown={(e) => {
              if (e.key === 'Enter') {
                e.preventDefault()
                handleAddText()
        ...
        {/* Font Size Selector */}
        <div className="flex items-center gap-1 bg-slate-900/80 border border-slate-700/80 rounded-xl px-2 py-1">
          <span className="text-[10px] text-slate-500 font-mono select-none">px</span>
          <input
            type="number"
            value={fontSize}
            ...
            min={8}
            max={144}
            className="w-10 bg-transparent text-xs font-mono font-bold text-slate-200 text-center focus:outline-none"
          />
        </div>
